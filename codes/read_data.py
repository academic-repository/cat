...
def get_data(data_path, n_labeled_per_class, unlabeled_per_class=5000, max_seq_len=128, model='bert-base-uncased', train_aug=False):
    """Read data, split the dataset, and build dataset for dataloaders.

    Arguments:
        data_path {str} -- Path to your dataset folder: contain a train_long.csv and test.csv
    ...
    train_df = train_df.sample(frac=1,random_state=66)
...
#         train_text[train_unlabeled_idxs], train_unlabeled_idxs, tokenizer, max_seq_len, Translator(data_path))
    ...
    test_dataset = loader_labeled(
        test_text[:2000], test_labels[:2000], tokenizer, max_seq_len)

    print("#Labeled: {}, Val {}, Test {}".format(len(
        train_labeled_idxs), len(val_idxs), len(test_labels[:2000])))
...
def get_data_pair(data_path, n_labeled_per_class, unlabeled_per_class=5000, max_seq_len=128, model='bert-base-uncased', train_aug=False):
    """Read data, split the dataset, and build dataset for dataloaders.
    ...
    train_df = pd.read_csv(data_path+'train.csv', header=None, sep='\t')
    train_df = train_df.sample(frac=1,random_state=66)
    test_df = pd.read_csv(data_path+'test.csv', header=None, sep='\t')
#     test_df = test_df.sample(frac=1)

    # Here we only use the bodies and removed titles to do the classifications
    train_labels = np.array([v-1 for v in train_df[0]])
    train_text = []
    train_text_list = [v for v in train_df[[2, 3]].values.tolist()]
    for each in train_text_list:
        try:
            tmp = '\t'.join(each)
            train_text += [tmp]
        except:
    ...
    test_text = np.array([v for v in test_df[2]])
    test_text = []
    test_text_list = [v for v in test_df[[2, 3]].values.tolist()]
    for each in test_text_list:
        try:
            tmp = '\t'.join(each)
            test_text += [tmp]
        except:
    ...
    # Build the dataset class for each set
    train_labeled_dataset = loader_labeled_pair(
        train_text[train_labeled_idxs], train_labels[train_labeled_idxs], tokenizer, max_seq_len, train_aug)
#     train_unlabeled_dataset = loader_unlabeled(
#         train_text[train_unlabeled_idxs], train_unlabeled_idxs, tokenizer, max_seq_len, Translator(data_path))
    val_dataset = loader_labeled_pair(
        train_text[val_idxs], train_labels[val_idxs], tokenizer, max_seq_len)
    test_dataset = loader_labeled_pair(
        test_text[:2000], test_labels[:2000], tokenizer, max_seq_len)
...
class loader_labeled(Dataset):
    ...
    def __getitem__(self, idx):
        if self.aug:
            ...
            length = len(tokens)
            encode_result = [101] + self.tokenizer.convert_tokens_to_ids(tokens) + [102]
            attention_mask = [1] * len(encode_result) + [0] * (self.max_seq_len - len(encode_result))
            padding = [0] * (self.max_seq_len - len(encode_result))
            encode_result += padding
            if len(encode_result) != self.max_seq_len or len(attention_mask) != self.max_seq_len:
                print('something wrong')
            return (torch.tensor(encode_result), torch.tensor(attention_mask), self.labels[idx], length)


class loader_labeled_pair(Dataset):
    # Data loader for labeled data
    def __init__(self, dataset_text, dataset_label, tokenizer, max_seq_len, aug=False):
        self.tokenizer = tokenizer
        self.text = dataset_text
    ...
    def __getitem__(self, idx):
        if self.aug:
            ...
            text1 = tmp[0]
            text2 = tmp[1]
            # self.tokenizer('asfas')
            tokens_dict = self.tokenizer.encode_plus(text=text1, text_pair=text2, add_special_tokens=True)
            encode_result = tokens_dict['input_ids']
            token_type_ids = tokens_dict['token_type_ids']
            if len(encode_result) > self.max_seq_len:
                encode_result = encode_result[:self.max_seq_len]
            if len(token_type_ids) > self.max_seq_len:
                token_type_ids = token_type_ids[:self.max_seq_len]
            encode_result = encode_result + [0] * (self.max_seq_len - len(encode_result))
            # tokens = self.tokenizer.tokenize()
            # if len(tokens) > self.max_seq_len-2:
            #     tokens = tokens[:self.max_seq_len-2]
            length = len(tokens_dict['input_ids'])
            token_type_ids = token_type_ids + [0] * (self.max_seq_len - len(token_type_ids))
            # encode_result = [101] + self.tokenizer.convert_tokens_to_ids(tokens) + [102]
            attention_mask = [1] * len(tokens_dict['input_ids']) + [0] * (self.max_seq_len - len(tokens_dict['input_ids']))
            # padding = [0] * (self.max_seq_len - len(encode_result))
            # encode_result += padding
            if len(encode_result) != self.max_seq_len or len(attention_mask) != self.max_seq_len:
                print('something wrong')
            return (torch.tensor(encode_result), torch.tensor(attention_mask), torch.tensor(token_type_ids), self.labels[idx], length)
