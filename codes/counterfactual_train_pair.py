...
#         val_loss, val_acc = validate(
#             labeled_trainloader, model, criterion, epoch, mode='Valid Stats', is_plot=True)
...
def train(labeled_trainloader, model, optimizer, scheduler, criterion, epoch, n_labels,
    ...
    if count == 0:
        for _ in tqdm(range(args.val_iteration)):
            ...
            try:
                inputs_x, attention_mask, token_type_ids, targets_x, inputs_x_length = labeled_train_iter.next()
            except:
                labeled_train_iter = iter(labeled_trainloader)
                inputs_x, attention_mask, token_type_ids, targets_x, inputs_x_length = labeled_train_iter.next()
            ...
            inputs_x, attention_mask, token_type_ids, targets_x = inputs_x.to(device), attention_mask.to(device), token_type_ids.to(device), targets_x.to(device)

            logits = model(inputs_x, x1_attention_mask=attention_mask, token_type_ids=token_type_ids, mode='pairs')
    ...
    for batch_idx in tqdm(range(args.adversial_training_iteration)):
        ...
        token_type_ids_a, token_type_ids_b = token_type_ids.to(device), token_type_ids[idx].to(device)
        #         print('input_a shape', input_a.shape, 'input_b shape', input_b.shape)
        ...
        '''factual learning'''
        logits = model(input_a, x1_attention_mask=attention_mask_a, token_type_ids=token_type_ids_a, mode='pairs')
        ...
        for _ in range(5):  # 优化10次lambda
            # Mix sentences' hidden representations
            logits_mix = model(x1=input_a, x1_attention_mask=attention_mask_a, token_type_ids = token_type_ids_a
                               ,token_type_ids2= token_type_ids_b
                               , x2=input_b, x2_attention_mask=attention_mask_b
                               , l=l, mix_layer=mix_layer, mode='pairs')
...
def validate(valloader, model, criterion, epoch, mode, is_plot=False):
    model.eval()
    with torch.no_grad():
        loss_total = 0
        total_sample = 0
        ...
        for batch_idx, (inputs, attention_mask, token_type_ids, targets, length) in tqdm(enumerate(valloader)):
            ...
            outputs = model(inputs, x1_attention_mask=attention_mask,token_type_ids=token_type_ids, mode='pairs')
